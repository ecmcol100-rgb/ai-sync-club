...
1. **스펙 v6 화면 수정 미반영** — 변경 이력 45~54항(육식→소고기, 4-i 문안 확정,
   전환 문장 공백 해소, 강조 축소, 대조표 확대, 가운데점 통일 등). 다음 작업
2. 오디오 미확보 — timing.ts가 임시 길이(5.5자/초 환산).
...
4. 개괄편과의 형식 불일치: 출처 표기(전체 vs 간략) / 출처 위치(우하단 고정,
   좌하단 이전안 보류)
5. 항목 명칭: 예고 "질병/건강법" 분리 vs 라벨 "질병·건강법" 병합 — v6에서도
   양표기 병존(라벨 자막 행 분리 / 비주얼 행 병합), 미해결
6. docs/CarnivoreColonCut2.mp4 — 참고 영상, 커밋 제외(미추적 유지 중)

*해결됨: 4-i 문안(v6 47항 확정 — 반영은 1번), 클로징 "구독+알림 설정"(v6 53항
승인), 스펙 부제 잔재(v6 교정), 직업 캡션·note 간격(v6 50항 = 하단 레이아웃
재설계로 반영 완료)*
