...
1. **스펙 v7~v9 화면 수정 미반영** — 4-i를 근접 체질 단일 메시지로
   (금 문장 삭제 — v8 68항, gold 통일 — v7 65항, 심화편 자막은 근접 화면에만 —
   v7 66항), 클로징에 마무리 문단 신설 + "정반대" 예고(v8 69항·v9 71항),
   길이 약 11:22 재배선(v9 73항). 다음 작업
...
4. 개괄편과의 형식 불일치: 출처 표기(전체 vs 간략) / 출처 위치(우하단 고정,
   좌하단 이전안 보류)
5. 항목 명칭: 예고 "질병/건강법" 분리 vs 라벨 "질병·건강법" 병합 — v9에서도
