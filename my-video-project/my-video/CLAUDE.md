...
1. **`8ch_mok_spec_v6.md`** — 목양·목음편 확정 스펙(조립 후 원장님 통합 검수
   반영본, 변경 45~54항). 최상위. 문안·소재 배제 목록·시리즈 구조(기본/오해/심화
   3부)의 근거. v5의 "심화 ①" 부제 잔재는 v6에서 교정됨
2. **`8ch_mok_tts_v3.md`** — 확정 낭독 대본(ElevenLabs 입력용).
   **내레이션 자막의 원천** — 화면 자막 cue 문안은 이 문서에서만 가져온다
3. **컴포넌트별 사양서 7종** — `*_spec.md` (2a_organ_array / safety_caption /
...
4. **`episode_assembly_spec.md`** — 조립 사양 + **부록 A(구현 기록·미해결 이슈)**.
   새 세션은 부록 A부터 읽을 것
5. **`8체질의학_개괄영상_제작스펙_v5.md`** — 개괄편(기본 ⓞ) 스펙. 확정본 참고용
...
- **`CONSTITUTION_ACCENTS` 확정 8색** (원장님 확정): 목양 하양 / 목음 붉은색 /
  ...
  수음 연한 주황. **칩(ConstitutionName)은 5곳 한정** (v6 57항):
  EpisodeTitleCard · ConstitutionTitleCard · ComparisonTable 헤더 ·
  PersonTypoCard 체질명 · 섹션2 도입. 기준: "제목·도입에서 체질을 소개하는
...
1. 오디오 미확보 — timing.ts가 임시 길이(5.5자/초 환산, 총 671초=11:11).
   대본은 `8ch_mok_tts_v3.md`로 확보, TTS 생성·실측 대기.
   실측 시 46항 삭제분 등 문단 단위 재분배도 함께 처리
2. 목음 붉은색과 경고색(medical·과잉 화살표) 구분 — 실제 합성에서 재확인 필요
3. 개괄편과의 형식 불일치: 출처 표기(전체 vs 간략) / 출처 위치(우하단 고정,
   좌하단 이전안 보류)
4. 항목 명칭: 예고 "질병/건강법" 분리 vs 라벨 "질병·건강법" 병합 — v6에서도
   양표기 병존(라벨 자막 행 분리 / 비주얼 행 병합), 미해결
5. docs/CarnivoreColonCut2.mp4 — 참고 영상, 커밋 제외(미추적 유지 중)

*v6 화면 수정(45~54·61항)은 2026-09-05 반영 완료 — 상세는 부록 A-5-1.
